# -*- coding: utf-8 -*-
# ===========================
# Enhanced SageMaker LLaMA 3B Fine-Tuning Script
# Continuous Training with Full Dataset Processing + Advanced Optimizations
# ===========================
...
# === Enhanced Configuration for SageMaker ===
CONFIG = {
    'BASE_MODEL_NAME': "unsloth/llama-3.2-3b-bnb-4bit",
    'HF_MODEL_NAME': "ali009eng/llama-3b-triage-classifier",
    'HF_MODEL_ID': "ali009eng/llama-3b-triage-classifier",
    'HF_TOKEN': "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",
    'MAX_SEQ_LENGTH': 2048,
    'LOAD_IN_4BIT': True,
    'CHUNK_SIZE': 3000,  # Smaller chunks for better memory management
    'BATCH_SIZE': 2,      # Reduced for stability
    'LEARNING_RATE': 1e-4,  # Lower learning rate for better convergence
    'CHECKPOINT_DIR': "./llama_checkpoints",  # ✅ SageMaker local path
    'DATASET_PATH': "./esi_data.csv",         # ✅ SageMaker local path
    'OUTPUT_DIR': "./llama_checkpoints/outputs",  # ✅ SageMaker local path
    ...
    'NUM_EPOCHS_PER_CHUNK': 3,  # More epochs per chunk
    'WEIGHT_DECAY': 0.05,  # Increased regularization
    'LR_SCHEDULER': "cosine_with_restarts",  # Better learning rate scheduling
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler('./llama_checkpoints/training.log'),
...
class EnhancedLoggingCallback(TrainerCallback):
    """Enhanced logging callback for better monitoring"""
    
    def on_step_end(self, args, state, control, **kwargs):
        if state.global_step % 10 == 0:  # Log every 10 steps
            memory_usage = get_memory_usage()
            logger.info(f" Step {state.global_step}: Memory: {memory_usage:.1f} MB, Loss: {state.log_history[-1].get('loss', 'N/A') if state.log_history else 'N/A'}")
...
def setup_environment():
    ...
    try:
        ...
        logger.info(" HuggingFace setup completed")
    ...
    if torch.cuda.is_available():
        ...
        logger.info(" GPU memory fraction set to 90%")
...
def load_model_with_proper_fallback():
    ...
    try:
        ...
        try:
            ...
            logger.info("Will create new enhanced LoRA adapters")
            model = base_model
...
def validate_and_augment_data(df):
    """Validate and augment training data for better generalization"""
    logger.info("🔍 Validating and augmenting data...")
    
    # Data validation
    initial_count = len(df)
    valid_data = df.dropna()  # Remove rows with missing values
    ...
    for _, row in tqdm(valid_data.iterrows(), desc="Augmenting data", total=len(valid_data)):
        ...
        if row['pain_level'] > 0 and np.random.random() < 0.5:
...
def format_patient_info_enhanced(row):
    """Enhanced patient information formatting for better learning"""
...
def prepare_dataset_enhanced(start_idx, end_idx, tokenizer):
    """Enhanced dataset preparation with validation split"""
    logger.info(f"Loading data from rows {start_idx} to {end_idx}")
    ...
    jsonl_path = Path(CONFIG['CHECKPOINT_DIR']) / f"triage_dataset_{start_idx}_{end_idx}.jsonl"
    ...
    for idx, (_, row) in enumerate(tqdm(enhanced_chunk.iterrows(), desc="Preparing conversations", total=len(enhanced_chunk))):
        ...
        if (idx + 1) % 1000 == 0:
            ...
            logger.info(f"Prepared {idx + 1} conversations")
    ...
    logger.info(f"Dataset prepared: {len(conversations)} conversations")
...
def setup_lora_enhanced(model):
    ...
    if not has_lora:
        logger.info("Applying enhanced LoRA adapters...")
        model = FastLanguageModel.get_peft_model(
            ...
            bias="lora_only",  # Train bias terms
            use_gradient_checkpointing="unsloth",
            random_state=3407,
            use_rslora=True,  # Enable RSLoRA for better performance
            loftq_config=None
        )
        logger.info("✅ Enhanced LoRA adapters applied (r=32, alpha=64)")
    else:
...
def get_enhanced_training_args():
    ...
    return TrainingArguments(
        ...
        fp16=True,
        bf16=False,  # Disable bf16 for better stability
...
def save_and_upload_model_enhanced(model, tokenizer, processed_rows, chunk_num, metrics=None):
    """Enhanced model saving with metrics and metadata"""
    timestamp = int(time.time())
    local_ckpt_dir = Path(CONFIG['CHECKPOINT_DIR']) / f"chunk_{chunk_num}_ckpt_{timestamp}"
    
    logger.info(f"Saving enhanced model to {local_ckpt_dir}")
    ...
    logger.info(f"✅ Enhanced model saved locally")
    ...
    try:
        ...
        commit_message = (
            f"Enhanced Chunk {chunk_num} completed - {processed_rows} rows processed\n"
            f"Training metrics: {metrics}\n"
            f"Enhanced LoRA config: r=32, alpha=64\n"
            f"Features: Data augmentation, Early stopping, Validation"
        )
        ...
        tokenizer.push_to_hub(
            CONFIG['HF_MODEL_ID'],
            token=CONFIG['HF_TOKEN'],
            commit_message=f"Enhanced tokenizer for chunk {chunk_num}"
        )
        
        logger.info("✅ Enhanced model uploaded to HuggingFace Hub successfully!")
...
def train_chunk_enhanced(model, tokenizer, start_idx, end_idx, chunk_num):
    """Enhanced training on a specific chunk of data"""
    logger.info(f" Starting enhanced training for chunk {chunk_num}: rows {start_idx} to {end_idx}")
    ...
    enhanced_logging = EnhancedLoggingCallback()
    
    trainer.add_callback(early_stopping)
    trainer.add_callback(enhanced_logging)
...
def main():
    """Main training function with continuous processing and enhancements"""
    logger.info("🚀 Starting Enhanced LLaMA 3B Fine-tuning on SageMaker")
    logger.info("✨ Features: Enhanced LoRA, Data Augmentation, Early Stopping, Validation")
    ...
    try:
        while current_idx < total_rows:
            ...
            logger.info(f"📦 Processing enhanced chunk {chunk_num}: rows {current_idx} to {end_idx}")
            ...
            if current_idx < total_rows:
                logger.info(f"⏳ {total_rows - current_idx} rows remaining. Continuing to next enhanced chunk...")
